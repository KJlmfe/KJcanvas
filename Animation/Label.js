...
var SHAPE_TEXT = "SimpleText";    //默认图新填充的文本内容
...
var SHAPE_FONT = '15px sans-serif';

Label = function(text,textColor,font) //矩形类(宽，高，位置，填充的文本内容)
{
	this.text = SHAPE_TEXT;  //填充内容
	this.textColor = SHAPE_TEXTCOLOR;  
	this.fontSize = SHAPE_FONT;
...
Label.prototype.draw = function(x,y) //在x,y地方画出该矩形
{
...
Label.prototype.clear = function() //橡皮擦擦掉矩形内部
{
	ctx.clearRect("",this.x,this.y);
}
Label.prototype.del = function()  //从画板上删除该矩形
{
	canvas.del(this);
	canvas.restore();
}
Label.prototype.move = function(x,y,speed,path) //移动一个巨型
{
	//设置目标位置、移动速度、移动路径
...
Label.prototype.timeOfMove = function(x,y,speed) //计算移动矩形的动画时间
{
	//设置目标位置
	this.aimX = x;
	this.aimY = y;
